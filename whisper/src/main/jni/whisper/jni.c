...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribeWithLang(
    ...
    params.print_progress = false;
    params.print_timestamps = false;  // Disable for speed
    params.print_special = false;
    ...
    params.offset_ms = 0;
    params.no_context = true;
    params.single_segment = true;     // +5-10% speed, single output segment

    // Performance optimizations for mobile devices
    params.greedy.best_of = 1;        // Reduce beam search candidates (default is 5)
    params.entropy_thold = 2.4f;      // Skip low-confidence segments
    params.logprob_thold = -1.0f;     // Log probability threshold
    params.no_timestamps = true;      // Disable timestamp computation for speed
    params.token_timestamps = false;  // No token-level timestamps needed
    params.no_speech_thold = 0.6f;    // Skip silence segments faster

    // Optimize audio_ctx based on actual audio length
    // Default is 1500 (30s), but we can reduce it for shorter audio
    // Must be multiple of 64 for performance
    // 16000 samples/sec, so audio_data_length / 16000 = seconds
    // 1500 audio_ctx = 30s, so audio_ctx = (seconds / 30) * 1500
    int audio_seconds = (audio_data_length + 15999) / 16000;  // Round up
    int optimal_ctx = ((audio_seconds * 50) + 63) & ~63;      // 50 ctx per second, round to 64
    if (optimal_ctx < 256) optimal_ctx = 256;                 // Minimum safe value (~5s)
    if (optimal_ctx > 1500) optimal_ctx = 1500;
    params.audio_ctx = optimal_ctx;

    LOGI("Audio length: %d samples (%ds), using audio_ctx=%d", audio_data_length, audio_seconds, optimal_ctx);

    whisper_reset_timings(context);

    LOGI("Transcribing with language=%s, translate=%d, threads=%d", lang, translate, num_threads);
    if (whisper_full(context, params, audio_data_arr, audio_data_length) != 0) {
        LOGI("Failed to run the model");
    } else {
        whisper_print_timings(context);
